...
fn write_context_index(root: &Path) -> AppResult<ContextGraph> {
    ...
    let readme_count = graph
        ...
        .filter(|node| node.kind == "readme")
        .count();
    let markdown_count = graph
        .nodes
        .iter()
        .filter(|node| node.kind == "readme" || node.kind == "note")
        .count();
    let folder_count = graph
        .nodes
        .iter()
        .filter(|node| node.kind == "folder" && node.path.as_deref() != Some("."))
        .count();
    ...
    content.push_str(&format!("markdown_count: {}\n", markdown_count));
    content.push_str(&format!("readme_count: {}\n", readme_count));
    content.push_str(&format!("folder_count: {}\n", folder_count));
    content.push_str("---\n\n");
    content.push_str("# Typeset Context\n\n");
    content.push_str("This file is generated by Typeset from managed Markdown files and LAYOUT.md metadata. It is optimized for fast agent lookup before opening full note bodies.\n\n");
    content.push_str("<!-- TYPESET_CONTEXT_BEGIN -->\n");
    content.push_str("```json\n");
    content.push_str(&json);
    content.push_str("\n```\n");
    content.push_str("<!-- TYPESET_CONTEXT_END -->\n\n");
    content.push_str("## Fast Search Guide\n");
    content.push_str("- Read the JSON block first for exact paths, headings, tags, and links.\n");
    content.push_str(
        "- Open full Markdown files only after selecting a relevant path from this index.\n",
    );
    content.push_str("- README nodes include short summaries; normal note nodes include headings, tags, links, timestamps, and size.\n\n");

    let mut readmes: Vec<&ContextNode> = graph
        .nodes
        .iter()
        .filter(|node| node.kind == "readme")
        .collect();
    readmes.sort_by_key(|node| node.path.clone().unwrap_or_default());

    content.push_str("## README Context\n");
    if readmes.is_empty() {
        content.push_str(
            "- No README.md files yet. All managed Markdown files are still indexed below.\n",
        );
    } else {
        for node in &readmes {
            write_context_node_outline(&mut content, node);
        }
    }

    let mut markdown_nodes: Vec<&ContextNode> = graph
        .nodes
        .iter()
        .filter(|node| node.kind == "readme" || node.kind == "note")
        .collect();
    markdown_nodes.sort_by_key(|node| node.path.clone().unwrap_or_default());

    content.push_str("\n## Indexed Markdown Files\n");
    if markdown_nodes.is_empty() {
        content.push_str("- No Markdown files indexed yet.\n");
    } else {
...
fn write_context_node_outline(content: &mut String, node: &ContextNode) {
    let path = node.path.as_deref().unwrap_or_default();
    let kind = if node.kind == "readme" {
        "README"
    } else {
        "note"
    };
    content.push_str(&format!("- [{}]({}) - {}\n", node.label, path, kind));
    ...
    for heading in node.headings.iter().take(12) {
        content.push_str(&format!("  - h{}: {}\n", heading.level, heading.text));
    }
    if node.headings.len() > 12 {
        content.push_str(&format!(
            "  - ... {} more headings\n",
            node.headings.len() - 12
        ));
    }
}
...
fn build_context_graph(root: &Path) -> AppResult<ContextGraph> {
    ...
    let mut indexed_notes: Vec<(String, LayoutEntry, String)> = Vec::new();
    ...
    for folder in &folders {
        ...
        for entry in layout.entries {
            ...
            let is_readme = is_readme_path(&entry.path);
            let node_id = if is_readme {
                context_readme_id(&entry.path)
            } else {
                context_note_id(&entry.path)
            };
            let summary = if is_readme {
                readme_summary(root, &entry.path)?
            } else {
                None
            };
            let node_kind = if is_readme { "readme" } else { "note" };
            let edge_kind = if is_readme { "has_readme" } else { "contains" };
            ...
            indexed_notes.push((folder_label.clone(), entry, node_id));
        }
    }

    for (folder_label, note, source_id) in &indexed_notes {
        for heading in &note.headings {
            let heading_id = format!("heading:{}#{}", note.path, heading.slug);
            upsert_context_node(
                &mut nodes,
                &mut node_positions,
                ContextNode {
                    ...
                    path: Some(format!("{}#{}", note.path, heading.slug)),
        ...
        for link in &note.links {
            ...
            let target_id = if is_readme_path(&target_path) {
                let readme_id = context_readme_id(&target_path);
                upsert_context_node(
                    &mut nodes,
                    &mut node_positions,
                    ContextNode {
...
fn read_layout_index(folder: &Path) -> AppResult<Option<LayoutIndex>> {
    ...
    let content = fs::read_to_string(path)?;
    let Some(json) = extract_json_block(&content, "TYPESET_INDEX_BEGIN", "TYPESET_INDEX_END")
    else {
        return Ok(None);
    };
    let index =
        serde_json::from_str(&json).map_err(|error| AppError::Workspace(error.to_string()))?;
    Ok(Some(index))
}
...
fn extract_summary(content: &str) -> Option<String> {
    ...
    if clean.is_empty() {
        ...
        Some(format!(
            "{}...",
            clean.chars().take(217).collect::<String>()
        ))
    } else {
...
fn resolve_context_link(folder_label: &str, target: &str) -> Option<String> {
    ...
    if without_anchor.is_empty()
        || without_anchor.contains("://")
        || !without_anchor.to_lowercase().contains(".md")
    {
        return None;
    }
...
mod tests {
    ...
    fn context_graph_indexes_readme_without_full_note_summaries() {
        ...
        assert!(private
            .headings
            .iter()
            .any(|heading| heading.text == "Secret Heading"));
        assert!(private.tags.iter().any(|tag| tag == "private"));
        assert!(private.links.iter().any(|link| link == "README.md"));
        assert!(graph
            .nodes
            .iter()
            .any(|node| node.id == "heading:Private.md#secret-heading"));
        assert!(graph.edges.iter().any(|edge| {
            edge.source == "note:Private.md"
                && edge.target == "readme:README.md"
                && edge.kind == "links_to"
        }));
    }
    ...
    fn generated_context_file_is_hidden_and_protected() {
        ...
        assert!(!tree
            .iter()
            .any(|node| node.name.eq_ignore_ascii_case(CONTEXT_FILE)));
